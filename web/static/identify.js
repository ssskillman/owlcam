(() => {
  ...
  const selection = document.querySelector("#identify-selection")
  const selectionSummary = document.querySelector("#identify-selection-summary")
  ...
  const resultsDisclaimer = document.querySelector("#identify-results-disclaimer")
  ...
  const identifyFiles = async (files) => {
    ...
    if (!response.ok) {
      ...
      const error = new Error(
        expected
          ? payload.detail || "We couldn't accept those photos."
          : "photo-processing server unavailable",
      )
  ...
  const renderCard = (item, jobOrigin, originalFile) => {
    const card = document.createElement("article")
    card.className = "identify-card"

    const retry = document.createElement("button")
    retry.type = "button"
    retry.className = "identify-quiet"
    retry.textContent = "Retry identification"
    retry.hidden = !originalFile
    retry.addEventListener("click", async () => {
      retry.disabled = true
      retry.textContent = "Retrying…"
      setStatus(`Retrying ${originalFile.name}…`)
      try {
        const payload = await identifyFiles([originalFile])
        const next = payload.results?.[0]
        if (!next) throw new Error("photo-processing server unavailable")
        card.replaceWith(renderCard(next, jobOrigin, originalFile))
        await loadSummary()
        track("animal_id_retry_completed", {})
        setStatus(`Retried ${originalFile.name}.`)
      } catch (error) {
        track("animal_id_retry_failed", {})
        setStatus(error.expected ? error.message : OFFLINE_MESSAGE)
      } finally {
    ...
    actions.append(right, wrong, retry)
    ...
    card.append(img, heading, score, alts, decided, actions, extra)
    return card
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (selection) selection.open = false
    setStatus("Analyzing photos…")
    if (loader) loader.hidden = false
    submit.disabled = true
    // Keep result-to-file retry mapping stable if more photos are added while
    // this request is in flight.
    const submittedFiles = [...selected]
    const started = performance.now()
    try {
      const payload = await identifyFiles(submittedFiles)
      const duration = Math.round(performance.now() - started)
      for (const [index, item] of (payload.results || []).entries()) {
        results.append(renderCard(item, origin, submittedFiles[index]))
        if (item.is_unknown) track("animal_id_unknown_returned", {})
      }
      if (resultsDisclaimer) {
        resultsDisclaimer.hidden = results.children.length === 0
      }
      await loadSummary()
      ...
      track("animal_id_failed", { file_count: submittedFiles.length })
      setStatus(error.expected ? error.message : OFFLINE_MESSAGE)
    } finally {
